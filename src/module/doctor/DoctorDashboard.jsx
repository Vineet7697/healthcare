    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-full text-white transition cursor-pointer ${
...
const StatCard = ({ title, value, icon, onClick }) => (
  <div
    onClick={onClick}
    className="bg-white p-4 rounded-xl shadow border flex justify-between items-center cursor-pointer hover:bg-gray-50"
  >
    <div>
      <p className="text-sm text-gray-500">{title}</p>
      <p className="text-2xl font-semibold text-gray-800">{value}</p>
    </div>
    <div className="text-[#2277f7] text-2xl">{icon}</div>
  </div>
);
...
const ActionBtn = ({ label, icon, onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className="flex items-center justify-center gap-3 p-4 bg-linear-to-br from-[#2277f7] to-[#52abd4] text-white rounded-xl cursor-pointer hover:opacity-90"
  >
    {icon}
    {label}
  </button>
